...
INSERT INTO Employees (EmployeeID, FirstName, LastName, Department, Salary)
VALUES
    (5, 'Nimesh', 'Pareek', 'HR', 50000.00),
    (6, 'Prem', 'Soni', 'IT', 60000.00);
  


-- 1.	Write a simple stored procedure named GetEmployeeCount that returns the total number of employees in the "Employees" table.
...
DECLARE @ans INT
EXECUTE GetEmployeeCount @employee_count = @ans OUTPUT
SELECT @ans AS EmployeeCOunt
Go

-- 2. Create a stored procedure named GetEmployeesByDepartment that takes a department name as input and returns all the employees
...
-- 3.Write a stored procedure named GetEmployeeInfo that takes an employee ID as input and returns the employee's name, 
...
-- 4. Create a stored procedure named InsertEmployee that inserts a new employee into the "Employees" table. However,
...
-- 5. Design a stored procedure named TransferFunds that transfers a specified amount from one bank account to another. 
...
-- 6. Write a stored procedure named GetEmployeeByFilter that allows the user to search for employees based on different filters,
...
-- 7. Implement a stored procedure named GetEmployeeHierarchy that takes an employee ID as input and returns the hierarchical
...
-- 8.	Create a stored procedure named InsertEmployees that takes a table-valued parameter containing employee data 
-- (employee ID, name, department, etc.) and inserts multiple employees into the "Employees" table in a single call.

-- Create a user-defined table type for the employee data
...
--@employee_id INT, @name VARCHAR(50), @dept VARCHAR(50)
--AS
--BEGIN
--INSERT INTO [EmployeeTableType]
--VALUES(@employee_id, @name, @dept)
--PRINT('INSERTED')
--END

--EXECUTE InsertEmployees 1, 'Nimesh', 'IT'

--SELECT *FROM EmployeeTableType

-- 9. Write a stored procedure named GetEmployeeProjects that takes an employee ID as input and returns two result sets:
...
AS
BEGIN
    -- Result set: Employee details and projects
    SELECT
        EP.EmployeeID,
        EP.FirstName,
        EP.LastName,
...
--	10.	Design a stored procedure named GetEmployeesByProject that takes a project ID as input and returns the employees who are 
...
AS
BEGIN
    -- Retrieve employees for the specified project
    SELECT
        EmployeeID,
        FirstName,
        LastName,
...
-- Declare and set the project ID
DECLARE @ProjectID INT;
SET @ProjectID = 102; -- Replace with the actual project ID
